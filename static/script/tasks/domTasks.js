...
function updateRewards(task) {
  let xp_reward = task["xp_reward"];
  let coin_reward = task["coin_reward"];
  ...
  add_coins(coin_reward * factor);
  add_xp(xp_reward * factor);
}
...
for (let i = 0; i < tasks.length; i++) {
  let task = tasks[i];
  console.log(task);
  taskVars[`task${task["task_id"]}`] = task;

  let varible = new ActionItem({
    label: task["task_name"],
    checkbox: true,
    xp_reward: task["xp_reward"],
    coin_reward: task["coin_reward"],
    clickfunc: () => updateRewards(task),
  });
